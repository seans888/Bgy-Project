    ...
    foreach($$object_name->relations as $rel_info)
    {
        if($rel_info['type'] == '1-M')
        {
            ...
            foreach($child->fields as $field_name=>$metadata)
            {
                ...
                if($metadata['control_type'] == 'upload')
                {
                    $var_file = 'cf_' . $child->table_name . '_' . $field_name;
                    $var_existing_file = 'existing_cf_' . $child->table_name . '_' . $field_name;
                    ////debug($_POST);
                    init_var($arr_form_data[$var_file]);
                    init_var($arr_form_data[$var_existing_file]);
                    
                    if(isset($_POST[$var_existing_file]))
                    {
                        //debug("Existing Image Set to POST");
                        $arr_form_data[$var_existing_file] = $_POST[$var_existing_file];
                        $arr_form_data[$var_file] = $_POST[$var_existing_file];
                    }
                    else
                    {
                        //debug("Existing Image NOT Set");
                    }
                    
                    if (isset($_FILES[$var_file]))
                    {
                        //debug("New Image Set from FILES");
                        //$arr_form_data[$var_file] = $_FILES[$var_file]['name'];
                    }
                    else 
                    {
                        //debug("NO New Image Set");
                    }
