...
class base_html
{
    ...
    function draw_file_upload($cobalt_text_field_label, $tf_control_name='', $upload_path='', $detail_view=FALSE, $draw_table_tags=TRUE, $extra='')
    {
        ...
        $display_name = substr($value, $this->upload_token_length);
    ...
    function draw_hidden($tf_control_name='', $value='')
    {
        ...
        echo '<input type=hidden id="' . $tf_control_name . '" name="' . $tf_control_name . '" value="' . $value . '">' . "\r\n";
    }
    ...
    function draw_radio_buttons($arrayItems, $cobalt_field_label, $form_control_name=null, $draw_table_tags=TRUE, $extra='')
    {
        ...
        for($a=0;$a<$numItems;++$a)
        {
            ...
            echo '<input type="radio" id="' . $form_control_name . '[' . $a . ']" name="' . $form_control_name . '" tabindex="' . $this->tabindex . '" value="' . cobalt_htmlentities($arrayItems['values'][$a]) . '" ' . $mark .' ' . $extra . '><label for="' . $form_control_name . '[' . $a . ']">' . $arrayItems['items'][$a] . '</label>' . $ending . "\r\n";
        }
...
*/
    ...
    function draw_text_field($cobalt_field_label, $tf_control_name='', $detail_view=FALSE, $control_type='', $draw_table_tags=TRUE, $extra='')
    {
        ...
        if($detail_view == TRUE && 
            ...
            $this->fields[$tf_control_name]['allow_html_tags'] == FALSE)
        {
            $value = cobalt_htmlentities($$tf_control_name);
        }
        else
